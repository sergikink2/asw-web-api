...
<body>
	...
	<div class="tres">
		...
		<h3>Edit issue</h3>
			<div id="container">
				<form action="/updateissue" class="form-register"  method="POST" id="form">

					<div class="input-group" id="name">
						<td>Title<span style="color:red">*</span>
						<td><input type="text" id="issuename" name="issuename" required value={{ .issuenameupdate}}></td>
					</div>

					<div class="input-group" id="description">
						<td> Description </td>
						<td><textarea type="text" id="issuedescription" name="issuedescription" value={{ .issuedescriptionupdate}}> </textarea></td>
					</div>
					
					<div class="input-group">
						<td><span class="input-group-addon" id="basic-addon1"> Assignee </span></td>
						<td><select style="width:900px" name="worker" id="worker">
              {{ range .worker }}
                <option> {{ .}} </option>
              {{end}}
						</select></td>
					...
					<div class="input-group">
						<td><span class="input-group-addon" id="basic-addon1"> Status </span></td>
						<td><select style="width:900px" name="status" id="status">
              {{ range .status }}
                <option> {{ .}} </option>
              {{end}}
						</select></td>
              ...
              {{end}}
					...
					<div id="savebutton" style="float:right">
						<td><button class="buttonSave" type="submit" value="SAVE"> Save </button></td>
					</div>	
				</form>
				<form action="/getlist">
